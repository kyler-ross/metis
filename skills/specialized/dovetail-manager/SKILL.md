...
**Location**: `.ai/scripts/dovetail-api.js`
...
**You**:
1. Run: `node .ai/scripts/dovetail-api.js projects list`
2. Parse JSON output
...
2. Run: `node .ai/scripts/dovetail-api.js projects create '{"name":"Q1 2026 User Research","description":"User feedback and research for Q1 2026 planning"}'`
...
**You**:
1. Run: `node .ai/scripts/dovetail-api.js search query '{"query":"checkout flow"}'`
...
**You**:
1. First search: `node .ai/scripts/dovetail-api.js search query '{"query":"mobile UX"}'`
2. Extract content IDs/text from results
3. Run: `node .ai/scripts/dovetail-api.js search summarize '{"content":["text1","text2","text3"],"length":"medium","style":"executive"}'`
4. Present summary
...
3. Run: `node .ai/scripts/dovetail-api.js insights create '{"title":"Users want faster checkout on mobile","description":"Analysis of user feedback showing demand for streamlined mobile checkout experience","project_id":"abc123","tags":["mobile","checkout","ux"]}'`
...
3. Run: `node .ai/scripts/dovetail-api.js notes import PROJECT_ID FILE_DATA`
...
2. Run: `node .ai/scripts/dovetail-api.js notes export --project-id PROJECT_ID --format csv`
...
2. Run: `node .ai/scripts/dovetail-api.js contacts create '{"name":"Jane Smith","email":"jane@example.com","company":"TechCorp"}'`
...
```bash
# Get token info
node .ai/scripts/dovetail-api.js token info
```

### Projects
```bash
# List all projects
node .ai/scripts/dovetail-api.js projects list

# List with pagination
node .ai/scripts/dovetail-api.js projects list --limit 20 --offset 0

# Get specific project
node .ai/scripts/dovetail-api.js projects get PROJECT_ID

# Create project
node .ai/scripts/dovetail-api.js projects create '{"name":"Project Name","description":"Description here"}'

# Update project
node .ai/scripts/dovetail-api.js projects update PROJECT_ID '{"name":"New Name"}'

# Delete project
node .ai/scripts/dovetail-api.js projects delete PROJECT_ID
```

### Insights
```bash
# List insights
node .ai/scripts/dovetail-api.js insights list

# Filter by project
node .ai/scripts/dovetail-api.js insights list --project-id PROJECT_ID

# Get specific insight
node .ai/scripts/dovetail-api.js insights get INSIGHT_ID

# Create insight
node .ai/scripts/dovetail-api.js insights create '{"title":"Title","description":"...","project_id":"...","tags":["tag1","tag2"]}'

# Update insight
node .ai/scripts/dovetail-api.js insights update INSIGHT_ID '{"title":"New Title"}'

# Delete insight
node .ai/scripts/dovetail-api.js insights delete INSIGHT_ID

# Import insights
node .ai/scripts/dovetail-api.js insights import PROJECT_ID FILE_DATA
```

### Notes
```bash
# List notes
node .ai/scripts/dovetail-api.js notes list

# Filter by project
node .ai/scripts/dovetail-api.js notes list --project-id PROJECT_ID

# Get specific note
node .ai/scripts/dovetail-api.js notes get NOTE_ID

# Create note
node .ai/scripts/dovetail-api.js notes create '{"title":"Title","content":"Content here","project_id":"..."}'

# Update note
node .ai/scripts/dovetail-api.js notes update NOTE_ID '{"title":"New Title","content":"Updated content"}'

# Delete note
node .ai/scripts/dovetail-api.js notes delete NOTE_ID

# Export notes
node .ai/scripts/dovetail-api.js notes export --project-id PROJECT_ID --format csv

# Import notes
node .ai/scripts/dovetail-api.js notes import PROJECT_ID FILE_DATA
```

### Search & AI
```bash
# AI-powered search
node .ai/scripts/dovetail-api.js search query '{"query":"search terms","project_id":"PROJECT_ID"}'

# AI summarization
node .ai/scripts/dovetail-api.js search summarize '{"content":["text1","text2"],"length":"short"}'
...
```bash
# List all tags
node .ai/scripts/dovetail-api.js tags list

# Filter by project
node .ai/scripts/dovetail-api.js tags list --project-id PROJECT_ID
```

### Highlights
```bash
# List highlights
node .ai/scripts/dovetail-api.js highlights list

# Filter by project or note
node .ai/scripts/dovetail-api.js highlights list --project-id PROJECT_ID
node .ai/scripts/dovetail-api.js highlights list --note-id NOTE_ID

# Get specific highlight
node .ai/scripts/dovetail-api.js highlights get HIGHLIGHT_ID
```

### Contacts
```bash
# List contacts
node .ai/scripts/dovetail-api.js contacts list

# Get specific contact
node .ai/scripts/dovetail-api.js contacts get CONTACT_ID

# Create contact
node .ai/scripts/dovetail-api.js contacts create '{"name":"Name","email":"email@example.com","company":"Company"}'

# Update contact
node .ai/scripts/dovetail-api.js contacts update CONTACT_ID '{"name":"New Name"}'

# Delete contact
node .ai/scripts/dovetail-api.js contacts delete CONTACT_ID
```

### Channels
```bash
# Create channel
node .ai/scripts/dovetail-api.js channels create '{"name":"Channel Name","project_id":"..."}'

# Update channel
node .ai/scripts/dovetail-api.js channels update CHANNEL_ID '{"name":"New Name"}'

# Delete channel
node .ai/scripts/dovetail-api.js channels delete CHANNEL_ID

# Add data to channel
node .ai/scripts/dovetail-api.js channels add-data CHANNEL_ID '{"data":"..."}'
```

### Data Operations
```bash
# Export data
node .ai/scripts/dovetail-api.js data export --project-id PROJECT_ID --format csv

# Import data
node .ai/scripts/dovetail-api.js data import PROJECT_ID FILE_DATA
```

### Files
```bash
# Get file
node .ai/scripts/dovetail-api.js files get FILE_ID
```
...
**User**: "What did users say about privacy features?"

**Agent**:
```
🔍 Searching Dovetail for "privacy features"...

Found 34 results:

Top Insights (5):
✓ Users want more control over data sharing
✓ Privacy settings are hard to find
✓ Transparency builds trust

Recent Notes (8):
→ Interview #52: Privacy concerns (2 days ago)
→ Survey results: Privacy preferences (1 week ago)

Key Highlights (21):
"I don't know what data you're collecting"
"Privacy policy is too long to read"
"Love the privacy features!"

Would you like me to:
A) Summarize all privacy feedback
