 ...
 *
 * Creates/updates ~/.cloaked-env.sh with exports needed by MCP servers
 ...
 *
 * Usage:
 *   node .ai/scripts/setup-shell-env.cjs           # Interactive setup
 *   node .ai/scripts/setup-shell-env.cjs --check   # Check current state
 *   node .ai/scripts/setup-shell-env.cjs --fix     # Auto-fix zshrc sourcing
 */
...
/**
 * Scan zshrc for hardcoded exports that should be in ~/.cloaked-env.sh
 */
function scanZshrc() {
  if (!fs.existsSync(ZSHRC_PATH)) {
    return { exports: {}, sourcesDotenv: false, sourcesCloakedEnv: false, content: '' };
  }
  ...
  const sourcesDotenv = content.includes('.ai/scripts/.env');
  const sourcesCloakedEnv = content.includes('.cloaked-env.sh');

  return { exports, sourcesDotenv, sourcesCloakedEnv, content };
}

/**
 * Generate ~/.cloaked-env.sh content
 */
function generateShellEnv(vars) {
  const lines = [
    '#!/bin/bash',
    '# Cloaked PM AI - Shell Environment Variables',
    ...
    '#',
    '# Node.js scripts use .ai/scripts/.env via dotenv instead.',
    '# Do NOT put Google OAuth, Dovetail, or other Node-only keys here.',
    '#',
...
function checkState() {
  ...
  // Check if ~/.cloaked-env.sh exists
  if (!fs.existsSync(SHELL_ENV_PATH)) {
    issues.push({
      severity: 'error',
      message: '~/.cloaked-env.sh does not exist',
      fix: 'Run: node .ai/scripts/setup-shell-env.cjs',
    });
  } else {
    info.push(`~/.cloaked-env.sh exists with ${Object.keys(shellEnvVars).length} var(s)`);
  }
  ...
  if (zshrc.sourcesDotenv) {
    issues.push({
      severity: 'warning',
      message: '~/.zshrc sources .ai/scripts/.env (should source ~/.cloaked-env.sh instead)',
      fix: 'Run: node .ai/scripts/setup-shell-env.cjs --fix',
    });
  }

  // Check if zshrc sources ~/.cloaked-env.sh (good)
  if (!zshrc.sourcesCloakedEnv) {
    issues.push({
      severity: 'error',
    ...
    info.push('~/.zshrc sources ~/.cloaked-env.sh');
  }
  ...
  if (Object.keys(zshrc.exports).length > 0) {
    issues.push({
      ...
      fix: 'Move to ~/.cloaked-env.sh and remove from ~/.zshrc',
    });
  }
  ...
  for (const [key, meta] of Object.entries(MCP_VARS)) {
    ...
    if (!inShellEnv && !inZshrc && !inProcessEnv) {
      issues.push({
        ...
        fix: `Add to ~/.cloaked-env.sh or get from ${meta.docs}`,
      });
    } else if (inShellEnv) {
      info.push(`${key}: set in ~/.cloaked-env.sh`);
    } else if (inZshrc) {
      info.push(`${key}: set in ~/.zshrc (should move to ~/.cloaked-env.sh)`);
  ...
  if (fs.existsSync(DOTENV_PATH)) {
    ...
    if (content.includes('\r')) {
      issues.push({
        severity: 'error',
        message: '.ai/scripts/.env has Windows line endings (CRLF)',
        fix: "Run: perl -pi -e 's/\\r\\n/\\n/g' .ai/scripts/.env",
      });
    }
  }
...
/**
 * Collect vars from all sources and write ~/.cloaked-env.sh
 */
...
/**
 * Fix zshrc: remove .env source, add .cloaked-env.sh source, remove hardcoded exports
 */
function fixZshrc() {
  if (!fs.existsSync(ZSHRC_PATH)) {
  ...
  // Add ~/.cloaked-env.sh sourcing if not present
  if (!content.includes('.cloaked-env.sh')) {
    ...
    const insertPoint = pathIdx >= 0
      ...
      : content.length;
    const sourceBlock = `
# Cloaked PM AI - MCP and shell credentials
# Managed by: node .ai/scripts/setup-shell-env.cjs
if [ -f "$HOME/.cloaked-env.sh" ]; then
  source "$HOME/.cloaked-env.sh"
fi
`;
    content = content.slice(0, insertPoint) + sourceBlock + content.slice(insertPoint);
    changes++;
    console.log('  Added ~/.cloaked-env.sh sourcing to ~/.zshrc');
  }
...
if (require.main === module) {
  ...
  if (args.includes('--check')) {
    ...
    const zshrc = scanZshrc();
    if (zshrc.sourcesDotenv || !zshrc.sourcesCloakedEnv || Object.keys(zshrc.exports).length > 0) {
      const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
      rl.question('  Update ~/.zshrc to source ~/.cloaked-env.sh? [Y/n] ', (answer) => {
